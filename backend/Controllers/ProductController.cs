...
namespace backend.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<ProductDto>>> Create([FromBody] CreateProductDto dto)
        {
            ...
            if (dto.MerchantIds != null && dto.MerchantIds.Any())
            {
                var merchants = await _context.Merchants.Where(m => dto.MerchantIds.Contains(m.MerchantId)).ToListAsync();
                foreach (var merchant in merchants)
                {
                    product.Merchants.Add(merchant);
                }
            }
        ...
        public async Task<ActionResult<ApiResponse<ProductDto>>> Update(int id, [FromBody] UpdateProductDto dto)
        {
            ...
            {
                product.Merchants.Clear();
                if (dto.MerchantIds.Any())
                {
                    var merchants = await _context.Merchants.Where(m => dto.MerchantIds.Contains(m.MerchantId)).ToListAsync();
                    foreach (var merchant in merchants)
                    {
